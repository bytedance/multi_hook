...
const char multi_hook_version[] = "1.0.0";
...
// user's func_addr as rcu pointer
// long hook_func0(long arg0, long arg1, long arg2, long arg3, long arg4, long arg5) {
// 	return hook_generic_func(&ctxs[0], arg0, arg1, arg2, arg3, arg4, arg5);
// }
#define define_hook_func(idx) \
long hook_func##idx(long arg0, long arg1, long arg2, long arg3, long arg4, long arg5) { \
...
struct hook_ctx_wrapper_t wrappers[MAX_HOOKS];
struct mutex wrappers_mutex;
				 ...
				 unsigned long user_func, const char *desc)
{
	...
	if ((ret =
	     hook_ctx_init(wrapper->ctx, wrapper->user_func,
			   wrapper->hook_func)) < 0)
		return ret;
...
struct hook_ctx_t *multi_hook_manager_get(unsigned long addr, const char *desc)
{
	...
	for (i = 0; i < MAX_HOOKS; i++) {
		if (wrappers[i].user_func == addr
		    && wrappers[i].user_ref_cnt > 0) {
			wrappers[i].user_ref_cnt++;
			pr_debug
			    ("multi_hook_manager: found existed ctx, new_user_ref_cnt: %d\n",
			     wrappers[i].user_ref_cnt);
			ctx = wrappers[i].ctx;
			goto out_unlock;
		}
...
int multi_hook_manager_put(unsigned long addr)
{
	...
	for (i = 0; i < MAX_HOOKS; i++) {
		...
		if (wrapper->user_func == addr && wrapper->user_ref_cnt > 0) {
			int new_user_ref_cnt = --wrapper->user_ref_cnt;
			if (!new_user_ref_cnt) {
...
int multi_hook_manager_init(void)
{
	int i;
	...
	mutex_init(&wrappers_mutex);

	return 0;
}

void multi_hook_manager_exit(void)
{
	int i;
				     ...
				     int index)
{
	...
	for (i = 0; i < HOOK_CTX_PREV_NUM; i++) {
		struct hook_func_t *hook_func = rcu_dereference(wrapper->ctx->prev_func[i]);
		if (hook_func)
			seq_printf(seq,
	...
	for (i = 0; i < HOOK_CTX_POST_NUM; i++) {
		struct hook_func_t *hook_func = rcu_dereference(wrapper->ctx->post_func[i]);
		if (hook_func)
			seq_printf(seq,
...
static int multi_hook_manager_proc_fs_open(struct inode *inode,
					   struct file *file)
{
	return single_open(file, multi_hook_manager_proc_fs_show, NULL);
}

static const struct file_operations multi_hook_manager_fops = {
	.owner = THIS_MODULE,
	.open = multi_hook_manager_proc_fs_open,
	.read = seq_read,
	.llseek = seq_lseek,
...
static struct proc_dir_entry *folder_entry, *stats_entry;
...
static int __init multi_hook_init(void)
{
	...
	stats_entry =
	    proc_create("stats", 0, folder_entry, &multi_hook_manager_fops);
	if (!stats_entry)
...
static void __exit multi_hook_exit(void)
{
	...
	remove_proc_entry("stats", folder_entry);
	remove_proc_entry("multi_hook_manager", NULL);
