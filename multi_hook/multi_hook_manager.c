...
#define MULTI_HOOK_VERSION "1.3.0"
...
static int __init multi_hook_manager_init(void)
{
	...
	for (i = 0; i < MAX_HOOKS; i++) {
		...
		wrappers[i].ctx->call_count = NULL;
...
static void multi_hook_manager_exit(void)
{
	...
	/* to ensure hook_generic_func and hook_ctx_put_destroy exit. */
	synchronize_rcu();
}
...
{
	int i;
	uint64_t call_count_sum = 0;

	if  (wrapper->ctx->call_count) {
		for_each_possible_cpu(i) {
			uint64_t* call_count_p = per_cpu_ptr(wrapper->ctx->call_count, i);
			call_count_sum += *call_count_p;
		}
	}

	seq_printf(seq,
		   "index: %d, user_ref_cnt: %d, user_func: %lx, hook_func: %lx, call_count: %llu, desc: %s\n",
		   index, wrapper->user_ref_cnt, wrapper->user_func,
		   wrapper->hook_func, call_count_sum, wrapper->desc);
